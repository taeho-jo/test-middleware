    ...
    if (isShare) {
      ...
      if (userInfo) {
        if (userIndicator) {
          ...
          const updateObject = {
            graph: saveObject?.graph,
            originData: saveObject?.originData,
            isShare: saveObject?.share ? saveObject?.share : saveObject?.isShare,
            isFilter: saveObject?.filter ? saveObject?.filter : saveObject?.isFilter,
          };
          dispatch(updateInitIndicator(updateObject));
          localStorage.setItem(userInfo?.userId, JSON.stringify(updateObject));
        } else {
