...
const CONTENTS = {
  goal: {
    image: decisionImage,
    title: '리서치 목표 = 유저 리서치를 통해 하고 싶은 의사결정',
  ...
  return (
    <FlexBox direction={'column'} width={'100%'} height={'500px'} style={selectContainer}>
      <div css={topContainer}>
        <Image width={404} src={CONTENTS?.goal?.image} alt={'aa'} />
      </div>
      <div css={bottomContainer}>
        {CONTENTS?.goal?.title && <span css={[heading2_bold, { marginBottom: '16px' }]}>{CONTENTS?.goal?.title}</span>}
        <span css={[body2_medium, { whiteSpace: 'pre-line' }]}>{CONTENTS?.goal?.guideContent}</span>
      </div>
    </FlexBox>
  );
};
